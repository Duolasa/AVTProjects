...
const void QuaternionManipulator::vPrint(const std::string s, const Vector& v)
{
	std::cout << s << " = (" << v.x << ", " << v.y << ", " << v.z << ", " << v.w << ")" << std::endl;
}

const bool QuaternionManipulator::vEqual(const Vector& v0, const Vector& v1)
{
...
const Vector QuaternionManipulator::mMultiply(Matrix m, Vector& v)
{
	Vector r;
...
const void QuaternionManipulator::mClean(Matrix m)
{
	for(int i = 0; i < 16; i++){
...
const void QuaternionManipulator::qToAngleAxis(const Quaternion& q, float& theta, Vector& axis)
{
...
const void QuaternionManipulator::qClean(Quaternion& q)
{
...
const float QuaternionManipulator::qQuadrance(const Quaternion& q)
{
	return q.t*q.t + q.x*q.x + q.y*q.y + q.z*q.z;
}

const float QuaternionManipulator::qNorm(const Quaternion& q)
{
...
const Quaternion QuaternionManipulator::qConjugate(const Quaternion& q)
{
...
const Quaternion QuaternionManipulator::qAdd(const Quaternion& q0, const Quaternion& q1)
{
	Quaternion q;
	q.t = q0.t + q1.t;
	q.x = q0.x + q1.x;
...
const Quaternion QuaternionManipulator::qMultiply(const Quaternion& q, const float s)
{
	Quaternion sq;
	sq.t = s * q.t;
	sq.x = s * q.x;
...
const Quaternion QuaternionManipulator::qMultiply(const Quaternion& q0, const Quaternion& q1)
{
	Quaternion q;
...
const void QuaternionManipulator::qGLMatrix(const Quaternion& q, float matrix[16]) {
	Quaternion qn = qNormalize(q);
...
const bool QuaternionManipulator::qEqual(const Quaternion& q0, const Quaternion& q1)
{
	return ( fabs(q0.t-q1.t) < qThreshold && fabs(q0.x-q1.x) < qThreshold &&
		     fabs(q0.y-q1.y) < qThreshold && fabs(q0.z-q1.z) < qThreshold );
}

const void QuaternionManipulator::qPrint(const std::string& s, const Quaternion& q)
{
...
void QuaternionManipulator::qtest4()
{
...
void QuaternionManipulator::qtest5()
{
	HEADER("TEST 5: LERP")
...
void QuaternionManipulator::qtest6()
{
	HEADER("TEST 6: SLERP")
